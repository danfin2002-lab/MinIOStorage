...
class FileService:
	...
	async def upload_file(self, data: FilePutSchema)->MetadataGetSchema:
		...
		if existing_hash is not None:
			raise FileAlreadyExistsException("A file with such contents already exists")
		...
		try:
			...
			raise FPutObjectException(str(err))
	...
	def get_metadata(self)->list[MetadataGetSchema]:
		try:
			...
			raise ListObjectsException(str(err))
			#raise HTTPException(status_code=500, detail=str(err))
	...
	async def delete_file(self, data: FileDeleteSchema):
		self.check_exists_file(data.destination_file)	
		try:
			client.remove_object(bucket_name, data.destination_file)
		except S3Error:
			raise DeleteFileException("Error while deleting")
	...
	def get_file(self, destination_file: str): #Должен быть на выходе либо файл для скачивания или ссылка что ли
		self.check_exists_file(destination_file)
	...
	def check_exists_file(self, destination_file: str):
		try:
